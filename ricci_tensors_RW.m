...
% tensors, and Ricci tensors for the Robertson-Walker metric. It returns the
% non-zero Ricci tensors.

%% define matrix g and the variables
syms t r theta phi;
syms a(t) k;
g = [-1 0 0 0; 0 (a(t))^2/(1-k*r^2) 0 0; 0 0 (a(t))^2*r^2 0; 0 0 0 (a(t))^2*r^2*(sin(theta))^2]; % matrix g for the Robertson-Walker metric
G = inv(g);
X=[t r theta phi];
...
for i = 1:4
    for j = 1:4
        for l = 1:4
            for k = 1:4
                Q = (1/2)*G(l,k)*(diff(g(j,k),X(i))+diff(g(i,k),X(j))-diff(g(i,j),X(k))); 
    ...
    for j = 1:4
        for k = 1:4
            for l = 1:4
                ...
                R_ijk_l = R_ijk_l + L_1 + L_3; %update riemann curvature tensor
...
disp('To call any of the following:');
